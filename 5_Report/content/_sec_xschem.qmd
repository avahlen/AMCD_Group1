...
Simulations at V\(_\text{DD}\)=1.5 V show ≈ 34 dB open-loop gain and a unity-gain frequency in the low-tens of MHz for 20 µA bias—adequate for preliminary filter studies yet visibly below the performance of Prof. Prettel’s cascode-enhanced reference OTA adopted later in the project.  Even so, building and characterising this core proved invaluable: it validated our GM/ID sizing workflow, and—most importantly—established the Xschem → Ngspice net-listing chain we now use for every subsequent test bench.
