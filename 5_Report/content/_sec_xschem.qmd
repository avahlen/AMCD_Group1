...
![OTA schematic.](../images/sec_xschem/ota-omproved.png){#fig-self-built-ota width=80% fig-alt="schematic of the five-transistor OTA"}
ota-improved.png
...
After experimenting with various combinations of transistor widths and lengths in simulation, we found that many trade-offs emerged between gain, speed, and voltage headroom—particularly for the cascode devices. Ultimately, we followed Prof. Prettel’s recommendations, assigning a reduced channel length of $L = 0.5\\,\\mu\\text{m}$ to the input differential pair and cascode devices ($M_{1/1C}$, $M_{2/2C}$, $M_{3/3C}$, $M_{4/4C}$) to maximize speed and transconductance. Meanwhile, the tail current source transistors ($M_5$, $M_6$) were given a longer channel length of $L = 5\\,\\mu\\text{m}$ to improve matching, suppress flicker noise, and ensure robust common-mode rejection.
...
```
The NGSpice block at the top of the schematic (see Figure @fig-biquad-xschem) includes the device model definitions, the simulation temperature (set to 27°C), and references to the SG13G2 PDK libraries. Parameterized values for transistor dimensions are declared at the beginning to allow flexibility when tuning device behavior:
```spice
.param lp="X"u wp="X"u l="X"u w="X"u
```

![gm‑C biquad design.](../images/sec_xschem/ota_gm_100u.png){#fig-biquad-xschem width=80% fig-alt="OTA design implemented in Xschem for gm-C filter integration."}
...
### Calculations for device Dimensions for a 1 kHz gm-C Biquad (100 µA bias) {#sec-sizing-100u}

For the final gm‑C biquad implementation, the tail-bias current of every OTA is fixed at 100 µA a constraint imposed by the design. At the same time, the natural frequency of the biquad must be 1 kHz, which sets a fundamental challenge. The pole of a gm‑C integrator is defined by the equation

$$
f_{0} = \frac{g_{m}}{2\pi C}
$$

which shows that for a given bias current, either the transconductance must be reduced, or the capacitance must be increased in order to reach low frequencies. Because reducing g_m further would push the transistors into *extremely weak inversion*, degrading noise and matching performance, the decision was made to accept larger-than-usual on-chip capacitors while keeping the MOSFETs operating in moderate to strong inversion. This approach helps preserve dynamic range and reduces flicker noise.

To determine the necessary transconductance for the OTA, previous measurements on the existing OTAs yielded an efficiency of approximately

$$
\frac{g_m}{I_D} \approx 10\;\text{V}^{-1}
\quad\text{at}\quad V_{\text{OV}} \approx 0.20\,\text{V}
$$

indicating moderate inversion. With a tail current of 100 µA, each side of the differential pair carries 50 µA, resulting in a transconductance of

$$
g_{m} = \left( \frac{g_m}{I_D} \right) I_D = 10\;\frac{1}{\text{V}} \cdot 50\;\mu\text{A} = 0.50\;\text{mS}
$$

Using this transconductance value, the required capacitance to reach a natural frequency of 1 kHz can be calculated from

$$
C = \frac{g_m}{2\pi f_0} = \frac{0.50\ \text{mS}}{2\pi \cdot 1\ \text{kHz}} \approx 80\ \text{nF}
$$

> **Area check** (SG13G2 MIM ≈ 2 fF/µm²):  
> 80 nF ≈ 40 mm² — too large for a single metal layer, so the capacitor bank must be distributed across multiple layers or placed off-chip during lab validation. OTA sizing remains valid.

To confirm the selected operating point, we verify the overdrive voltage using the standard saturation-region relation:

$$
V_{\text{OV}} = \frac{2I_D}{g_m} = \frac{2 \cdot 50\ \mu\text{A}}{0.50\ \text{mS}} = 0.20\ \text{V}
$$

This matches the earlier assumption. Now, assuming a mobility-capacitance product of \( \mu_n C_{\text{ox}} \approx 200\ \mu\text{A/V}^2 \), the required width-to-length ratio becomes

$$
\frac{W}{L} = \frac{2I_D}{\mu_n C_{\text{ox}} V_{\text{OV}}^2} = \frac{100\ \mu\text{A}}{200\ \mu\text{A/V}^2 \cdot 0.04} = 12.5
$$

To simplify the layout and ensure symmetry and good matching, a unified transistor geometry is chosen, meaning all NMOS devices share the same dimensions, and likewise for all PMOS devices. The final device dimensions are summarised in the following table:

| Device set      | \(L\) | \(W\)  | Comment                                                                 |
|------------------|-----:|------:|-------------------------------------------------------------------------|
| **NMOS (all)**   | 1 µm | 12 µm | \(W/L \approx 12\) yields \(g_m \approx 0.50\ \text{mS}\) per OTA branch |
| **PMOS (all)**   | 1 µm | 30 µm | Width scaled ×2.5 to compensate lower hole mobility for accurate mirroring |
| **Tail devices** | 5 µm | 12 µm | Long channel boosts \(r_o\) and reduces flicker noise                   |

A common \( L = 1\ \mu\text{m} \) is used for all core devices to keep layout clean and compact, while the tail pair uses a longer length \( L = 5\ \mu\text{m} \) to achieve higher output resistance and suppress noise.

---

**Summary**

- **Bias current:** \( I_{\text{tail}} = 100\ \mu\text{A} \) (fixed)  
- **Transconductance per branch:** \( g_m \approx 0.50\ \text{mS} \)  
- **Capacitance per integrator:** \( C \approx 80\ \text{nF} \) → \( f_0 \approx 1\ \text{kHz} \)  
- **Unified transistor geometry:** NMOS 12 µm / 1 µm, PMOS 30 µm / 1 µm

This configuration achieves the desired 1 kHz pole with a fixed 100 µA bias in Theory. Although the required capacitors are large, they can be realised using stacked MIM layers or temporarily implemented off-chip during prototyping. Unified dimensions for NMOS and PMOS devices simplify common-centroid layout, ensuring excellent matching and layout efficiency across the four OTA blocks used in the gm‑C biquad filter.
