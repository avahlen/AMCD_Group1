...
Both of this sources can be explained by Ohms-Law: $R = \fraq{U}{I}$. For example the voltage source: There is no influence of the load, so $R$ may change and its value is unknown to the source. So to keep the voltage stable we can only change the current.
:::

### Operational Transconductance Amplifier (OTA)
In the design process only OTAs will be used, so the focus of this chapter will be on them. The operational transconductance amplifier puts out out a current, Unlike Voltage-Feedback Amplifiers REF:KAPITEL_VFA, proportional to its input voltage. In other words an OTA is a voltage controlled current source. 

The word transconductance is
