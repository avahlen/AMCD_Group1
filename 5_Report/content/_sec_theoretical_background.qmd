...
:::{.callout-warning}
wegen dem doppel-slash, bekomme ich von quarto beim kompilieren eine warnung
:::
...
*Saturation* 
Saturation is a specific operation mode of a transistor. Its also called "ON mode", because a channel between drain and source is created. This state is reached when the voltage $V_{DS} >= (V_{GS} - V_{th})$. When this happens the drain-source current I_DS gets stable (saturates) and becomes nearly indipendet of V_DS. Still it can be very well controlled via V_GS, due to an effect called "pinch-off" (\ref{https://en.wikipedia.org/wiki/MOSFET}).

The $g_\mathrm{m}/I_\mathrm{D}$ method is primarly inteded to be used in saturated MOSFETs. To ensure this we keep the drain-source voltage as $V_{ds} = V_{DD} / 2$.
