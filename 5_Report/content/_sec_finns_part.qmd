## Preamble to the Schematic

Before designing it was necessary to understand the main workflow of the
tool chain. The schematic design program is called Xschem. This tool
allows for a graphical representation of the netlist, like other
discreet ECAD software. The main difference between discreet tools and
integrated tools like Xschem are the devices that are placed within the
schematic.

While in discreet circuit design all ICs are just "blackbox"-symbols,
Xschem introduces a concept called hierarchical design. As most
integrated circuits are made from mosfets, resistors and capacitors,
these are within the symbol of for example the OTA. This increases the
complexity a lot especially when naming conventions are not followed. As
a guideline for naming conventions we used the "designers etiquette"
provided by Professor Pretel.

## Designing the ideal circuit

The original circuit was copied out of the ASLK Pro Board Manual
Experiment 4. This source provided both the ideal schematic and the
means to calculate the resistive and reactive elements. The calculation
of the capacitors and some of the resistors is based on the following
formulas:

In contrast to board level design simulating the schematic meant
replacing the discrete operational amplifiers with small signal models.
One of the lab tutors suggested using voltage controlled current sources
as they were easier to handle.

## Designing the real circuit

While the ideal circuit was based an ideal voltage controlled current
source, the real circuit needed to implement these as OTAs at a lower
hierarchal level. The current source was replaces by the 5T- OTA. This
five-transistor model is the smallest implementation of an OTA. The
schematic was sourced out of the script by Professor Pretel. This
implementation is based on 3 parts.

![](../images/sec_finns_property/image1.png){width="3.84in"
height="2.5903729221347334in"}

The OTA consists of 3 stages. M1 and M2 form the differential input
pair. They are powered by a current mirror which is formed by M3 and M4.
This allows the positive input to mirror its current to summative point
at the drain of M4. To bias the circuit the gate of the transistor M5 is
connected to a diode connected Mosfet. This allows for a stable current
to power the biasing transistor M5.
...
## 1.1 Introduction into the fundamentals of integrated layouting. 
...
source and in a preview status.
...
To make a device first the FOX layer needs to be opened. That is done by
...
![](../images/sec_finns_property/image2.png){width="6.225048118985127in"
...
source regions of the nmos.

![A screenshot of a computer screen AI-generated content may be
incorrect.](../images/sec_finns_property/image4.png){width="4.067361111111111in"
...
the active layer but pSD layer is used to implant the p+ region.

![](../images/sec_finns_property/image5.png){width="6.929376640419948in"
height="2.365990813648294in"}

Figure : Side view of an PMOS transistor
...
removing the insulator over these regions. Note that the first
...
and placing in an conductor like tungsten the opening.

![](../images/sec_finns_property/image6.png){width="6.109148075240595in"
height="3.931312335958005in"}

Figure : Metal connection to the drain and source contacts
...
acquired in this chapter can be mapped to this technology. The figure 5
...
![](../images/sec_finns_property/image8.png){width="6.989583333333333in"
height="1.4267399387576554in"}

Figure : Layer comparison
...
The schematic outlined by Professor Pretel consists of 13 N- and PMOS
...
python /foss/pdks/ihp-sg13g2/libs.tech/klayout/tech/lvs/run_lvs.py
\--netlist= YOUR_PROJECT.spice \--layout= YOUR_PROJECT.gds
\--run_dir=LVS/
...
found the "*SG13G2_os_layout_rules.pdf*" on the Github of IHP (see).
...
contacts this need to be done by hand. See figure 5 for a DRC clean
connection of the gate of an NMOS device.

![](../images/sec_finns_property/image9.png){width="4.967361111111111in"
height="3.186608705161855in"}

Figure : Example DRC clean connection of the gate
...
#### 1.3.1 Tips and tricks for the Layout Process. 
...
**Remarks for those who come from EDA software like KiCad. These might
...
### 1.3.2 Working with LVS during layouting
...
p-substrate (see Figure 7). Please note that the size of the masks shown
...
![](../images/sec_finns_property/image11.png){width="6.122449693788276in"
height="3.1256944444444446in"}

Figure : Tying down different types of bulks. Not to scale.
...
### 1.3.3 Working with DRC during layouting. 
...
As described in chapter (link to chapter) the "Marker Database Browser"
